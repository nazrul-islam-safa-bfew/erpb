<form name="searchIOW" action="./index.php?keyword=mdview+IOW" method="post">
<table width="90%"  align="center" class="dblue">
 <tr><td bgcolor="#0066FF" colspan="5" align="center"><font class="englishhead">All Item of Work (IOW) details</font></td></tr>
 <tr>
 <?
	 if($_GET["selectedPcode"])
	 	$selectedPcode=$_GET["selectedPcode"];
...
// Approved by MD Approved by Mngr P&C
 ?>
     <td><input type="radio" name="status" <? echo $r1;?> value="Forward to MD">Waiting for MD's Approval (<font color="#f00"><? echo countiow("Forward to MD",$selectedPcode,"mdview IOW","MD");?></font> nos)</td>
    <td><input type="radio" name="status" <? echo $r3;?> value="Approved">Approved (<font color="#f00"><? echo countapviow("Approved",$selectedPcode);?></font> nos)</td>
    <td><input type="radio" name="status" <? echo $r4;?> value="Completed">Completed (<font color="#f00"><? echo countiow("Completed",$selectedPcode);?></font> nos)</td>

</tr>
<tr>
     <td><input type="radio" name="status" <? echo $r2;?> value="Forward to PM">Waiting for Mngr. P&C's Checking (<font color="#f00"><? echo countiow("Forward to PM",$selectedPcode);?></font> nos)</td>
     <td><input type="radio" name="status" <? echo $r7;?> value="Raised by CM">Waiting for Mngr. C.M's Checking (<font color="#f00"><? echo countiow("Raised by CM",$selectedPcode);?></font> nos)</td>
     <td><input type="radio" name="status" <? echo $r5;?> value="Not Ready">Under Preparation (<font color="#f00"><? echo countiow("Not Ready",$selectedPcode);?></font> nos)</td>
</tr>
...
<tr><td colspan="2">Select Project: <select name="selectedPcode" class="selectedPcode">
...
{
		$itemCounter=the_iow_counter($r3,$typel[pcode],$iow,$status);
...
$sqlp.= " AND `position` not LIKE '888.%' ";
...
<tr bgcolor="#0066FF">
 <th height="30" class="th1">Project</th> 
...
<? 
$i=1;
while($iow=mysqli_fetch_array($sqlrunp)){ $i++;?>
...
*/
$sqlp2 ="SELECT * FROM `dma` WHERE  `dmaiow` LIKE '$iow[iowId]' order by dmaItemCode ASC";
// echo $sqlp2;
$directCost1 = 0;
$sqlrunp2 = mysqli_query($db, $sqlp2);
while($iowResult=mysqli_fetch_array($sqlrunp2))
{ 
  $directCost1+=$iowResult[rate]*$iowResult[dmaQty];	 
}
?>
 ...
 Estimated Dir. Exp. <font class="out"><? apv($selectedPcode,$iowid)//echo number_format(($directCost1/$totalCost1)*100);?>%,</font> Approved  <font class="out"><? echo number_format(($directCost/$totalCost)*100);?>%</font> 
<?php }
?>
 <br>
...
<?php 
if($iow["iowType"]=="2"){
		$amountC=calculateDirectCost($iow[iowProjectCode],$iowid);
	 ?>
	 <div align=right style="display:inline-block; float:right; font-weight:100;">
		Approved amount Tk. <span><?php echo $amountC[0]; ?></span> is <span><?php echo $amountC[1]; ?></span>% of Bid Total
	 </div>
<?php } ?>
</td> 
...
?>
<tr  <? if($iow["iowStatus"]=='noStatus')echo "style='font-weight: bold; border:1px solid #A99600;";  if($iow["position"]=="999.000.000.000")echo "background:#f00;color:#fff;'"; else echo "background:#ffc;'"; ?>>
 <td width="10"><? echo $iow[iowProjectCode];?></td>
 <td width="100" >

  </td>
 <td <? if($iow["iowStatus"]=='noStatus')echo " colspan='5'";?>><? echo  "<span style='    color: #f00;
    font-weight: normal;
    font-size: 14px;'>".$positionVal."</span> ".$iow[iowDes];?>
	 
<?php if($iow["position"]=="999.000.000.000"){
