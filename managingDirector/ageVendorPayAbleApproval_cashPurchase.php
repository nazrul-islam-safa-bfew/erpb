...
if($_POST['total_row'])
{
	...
	if($_POST['approved']=='Approved' || $_POST['approved']=='Forward For Approval')
		if($amount)
		{
	...
	{
		if(getPoTypePOSL($posl)==1 || getPoTypePOSL($posl)==2 || getPoTypePOSL($posl)==3){
			$approval_sql="delete from vendorPaymentApproval where posl='$posl' and vid='$vid_temp' and ISNULL(approved)";
			mysqli_query($db, $approval_sql);
		}
	}
...
?>
	...
	<form name="searchBy" action="./index.php?keyword=aged+vendor+payable+approval+cp" method="post">

	<table width="600" align="center" class="ablue">
		<tr>
			<td colspan="3" align="right" class="ablueAlertHd">Cash Purchase Approval</td>
		</tr>
<?
...
/* and vid in (select vid from porder where posl like '%')*/
...
if($loginDesignation=="Chairman & Managing Director")
	echo "--(".vendor_payable_approved_counter($typel[vid],$_POST["pcode"],"cp")." nos) ";
echo "</option>  ";
 }
?>
	</select>
 ...
 while($typel= mysqli_fetch_array($sqlrunp))
{
 ...
 if(vendorApprovalCounterProject($typel[pcode],"cp")>0)echo " style=\"background:#ddd; color:red; font-size:14px;\" ";
 echo ">$typel[pcode]--$typel[pname]         --(".vendorApprovalCounterProject($typel[pcode],"cp")." nos)  </option>  ";
 }
?>
	</select>
				</td>
...
elseif($Status=="Short by vendor")$sql.=" ORDER by field(porder.vid, '99', '85') asc ,vendor.vname ASC";
...
if(isFullpaid($mr[posl])) {
    //echo "<p>full paid $mr[posl]</p>";
    continue;
}
...
<?php
if(check_posl_approved($mr[posl])){
...
// 	chalan pdf
		...
		$chalan_sql="select reference,pdf_files from store$location where paymentSL='$mr[posl]' and pdf_files!=''";
		$chalan_q=mysqli_query($db,$chalan_sql);
if(mysqli_affected_rows($db)>0)
	echo "<div
	style='display: inline-block; border-radius: 5px; font-size: 12px; margin-top:2px;'
	>Challan/Voucher:";
		while($chalan_row=mysqli_fetch_array($chalan_q)){
			$arra=json_decode($chalan_row[pdf_files]);
			?>
...
$sqlp = "1 SELECT * from `popayments` WHERE (posl LIKE '". $mr[posl]."') AND (paidAmount < totalAmount) order by popID ASC ";
//echo $sqlp;
$sqlrunp= mysqli_query($db, $sqlp);
//echo mysql_error();
...
<? 
 ...
 echo number_format($currentPayable,2)."<br>";
						...
						<td align="right"><input type="text" style="text-align:right" id="approved_amount_<?php echo $r; ?>" name="approved_amount_<?php echo $r; ?>" value="<?php if($get_row_data['amount']){echo $get_row_data['amount'];$grand_amount=$get_row_data['amount']+$grand_amount;} else echo 0;?>"
								<?php if(!check_posl_approved($posl))echo "readonly"; ?> class="approved_amount" /></td>
					</tr>
...
if($cp_count==0){
	...
	echo "<tr>
	<td></td>
	<td align=right><b>$stt1</b></td>
	<td align=right><b>$stt2</b></td>
	<td align=right><b>$stt3</b></td>
	<td align=right><b>$stt4</b></td>
	<td align=right><b>$stt5</b></td>
	<td align=right><b>$stt6</b></td>	
	</tr>";
...
while($mr=mysqli_fetch_array($sqlq)){} 
				if($loginDesignation=="Chairman & Managing Director"){
					...
					$approvedText="Forward For Approval";
				}
				?>
						<tr>
							<td colspan="5"></td>
							...
							<td align="right"><input type="submit" value="<?php echo $approvedText; ?>" id="approved_button" name="approved" <?php if(($grand_amount<=0 && $loginDesignation=="Chairman & Managing Director") || ($grand_amount>0 && $loginDesignation=="Procurement Executive"))echo 'disabled="disabled"'; ?> onclick="document.getElementById('d_approved').value='';" />
								<input type="hidden" value="<?php echo $r; ?>" name="total_row" />
							</td>
							<td>&nbsp;<span>Total:</span><strong style="color:#FF0000; text-align:center; margin-left:50px; text-align:right" id="final_total_amount"><?php if($grand_amount)echo number_format($grand_amount,2); else echo 0;?></strong></td>
						</tr>
						<script type="text/javascript">
