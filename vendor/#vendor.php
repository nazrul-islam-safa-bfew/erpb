...
<tr>
    <td  class="vendorTdl"> Vendor Type</td> 
	  ...
	  <p><input type="radio" name="type" value="0" <? if($vendor[type]=='10') echo 'SELECTED';?>>Manufacture/ Importer/ 1st Class national contractor/ Monopolist/ MNC (10 Points)</p>
<p>	       <input type="radio" name="type" value="1" <? if($vendor[type]=='5') echo 'SELECTED';?>>Distributor/ Wholeseller/ 2nd class & regional contractor (5 points),</p>
<p>		   <input type="radio" name="type" value="2" <? if($vendor[type]=='0') echo 'SELECTED';?>>Retailer/ Local Vendor/ Local contractor with complete setup (0 points)</p>
<p>			<input type="radio" name="type" value="3" <? if($vendor[type]=='-10') echo 'SELECTED';?>>Agents (opportunist)/ 1 man company (-10 points),</p>
</div>
       </td>
</tr>	
...
<tr>
	...
	<td width="34%">
	  <div style="margin:5px 0px;" class="radio_group" >
               ...
               <p> <input  onchange="selector1(document.ven,document.ven.quality.options[document.ven.quality.selectedIndex].value)" type="radio" name="quality"  value="-2" <? if($vendor[quality]=='10') echo 'SELECTED';?>>Premium vendor (within top 10%) (10 points)</p>
<p>                 <input  onchange="selector1(document.ven,document.ven.quality.options[document.ven.quality.selectedIndex].value)" type="radio" name="quality"  value="0" <? if($vendor[quality]=='5') echo 'SELECTED';?>>Reputed & well-known vendor (between top 11% to 25%) (5 points)</p>
       <p>          <input  onchange="selector1(document.ven,document.ven.quality.options[document.ven.quality.selectedIndex].value)" type="radio" name="quality"  value="-2" <? if($vendor[quality]=='0') echo 'SELECTED';?>>General vendor(0 points)</p>
              <p>   <input  onchange="selector1(document.ven,document.ven.quality.options[document.ven.quality.selectedIndex].value)" type="radio" name="quality"  value="3" <? if($vendor[quality]=='-10') echo 'SELECTED';?>>Vendor with bad reputation  (-10 points)</p>
    </div></td>
...
<tr>
	...
	<td><select name="reliability" onchange="selector2(document.ven,document.ven.reliability.options[document.ven.reliability.selectedIndex].value)" >
	<option value="0" <? if($vendor[reliability]=='10') echo 'SELECTED';?>>Able to meet delivery schedule, able to influence market (10 points)</option>
	   <option value="6" <? if($vendor[reliability]=='5') echo 'SELECTED';?>>Able to meet delivery schedule, able to support contingency events (5 points)</option>
	    <option value="8" <? if($vendor[reliability]=='0') echo 'SELECTED';?>>Able to meet delivery schedule, limited ability to support contingency events (0 points)</option>
                 <option value="10" <? if($vendor[reliability]=='-10') echo 'SELECTED';?>>No visible assurance of ability to meet delivery schedule (-10 points)</option>
...
<tr>
	...
	<td><select name="experienceM" onchange="selector4(document.ven,document.ven.experienceM.options[document.ven.experienceM.selectedIndex].value)" >
        <option value="0" <? if($vendor[experienceM]=='10') echo 'SELECTED';?>>Above 10 years of continous operation (10 points)</option>
        <option value="-2" <? if($vendor[experienceM]=='5') echo 'SELECTED';?>>Less than 10 years  of continous operation (5 points)</option>
        <option value="3" <? if($vendor[experienceM]=='0') echo 'SELECTED';?>>Less than 3 years  of continous operation (0 points)</option>
        <option value="5" <? if($vendor[experienceM]=='-10') echo 'SELECTED';?>>No Experience (-10 points)</option>
      </select></td>
...
<tr>
	...
	<td><select name="experienceB" onchange="selector5(document.ven,document.ven.experienceB.options[document.ven.experienceB.selectedIndex].value)" >
                 <option value="0" <? if($vendor[experienceB]=='10') echo 'SELECTED';?>>Above 5 years with no records of failure (10 points)</option>
                 <option value="-2" <? if($vendor[experienceB]=='5') echo 'SELECTED';?>>Less than 5 years  with no records of failure (5 points)</option>
                 <option value="3" <? if($vendor[experienceB]=='0') echo 'SELECTED';?>>No Experience with BFEW (0 points)</option>

		</select>
			              <p style="color:#000000" ><input onChange="theTitle(this,document.ven.experienceB)" type="checkbox" value="5" <? if($vendor[experienceB]=='-10') echo 'SELECTED';?> id="theValue" title=""/>Failure experienced with BFEW <i>(failed to maintain delivery schedule with materials, equipments or
...
<tr>-->
  ...
  <td> 
   <? if($vendor[advance]=='-5') $c1='CHECKED';
      ...
      <input type="radio" name="advance" value="0" <? echo $c1;?> onClick="credit0();"> Yes (0 points)</td>
	<td><input type="text" name="advanceText" value="<? echo $vendor[advanceText];?>" disabled class="disabled" style="width:470px"></td>
</tr>  
...
<tr>
 ...
 <p  style="display:none"><input type="radio" name="cfacility" value="0" <? echo $c1;?> onClick="credit();" >   No (0 points, OPEN MARKET) </p>
  <input type="radio" name="cfacility" value="10" <? echo $c3;?> onClick="credit();"> Bill-to-Bill (10 points)
  <input type="radio" name="cfacility" value="4" checked="" onclick="credit();"> Yes, Credit Limit Tk.
   <input type="text" size="10" name="camount" value="<? echo $vendor[camount];?>" style="text-align:right" disabled  class="disabled"> for 
  <input type="text" size="5" maxlength="3" name="cduration" value="<? echo $vendor[cduration];?>" style="text-align:right"  disabled class="disabled"> Days (0 points)
  </td>
</tr>  
<tr >
  <td  class="vendorTdl"> Attachment</td>  
  <td> <input type="file" name="quoUpload"></td> 
</tr>
<tr>
...
<tr>
    <td  colspan="3" align="center"> <input type="submit" value="Save" name="saveVendor" class="vendor" onMouseOver="this.className='vendorhov'" onMouseOut="this.className='vendor'" onclick="checkThisOut();"></td>
</tr>	
<script type="text/javascript">
