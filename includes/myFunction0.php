...
function getEquipmentFrequencyEqCode($iowCode){
	...
	$q=mysqli_query($db,$sql);
	$row=mysqli_fetch_array($q);
	return ["maintenanceFrequency"=>$row[maintenanceFrequency],"eqItemCode"=>$row[eqItemCode],"measureUnit"=>$row[measureUnit] ];
}
...
function getAllProblemData($problemDataSingle=null){
	$problemData=["Engine problem","Gearbox problem","Brake/clutch problem","Abnormal tire wear","Electric problem","Suspension/chassis/differential problem","Hydraulic problem","Denting & painting problem"];
...
function maintenanceHeadSql($firstOctetOnly=false,$like=false,$prefix="",$delimiter=","){
	foreach(maintenanceHead() as $mntnc){
		if($firstOctetOnly){
			if($like)
				$mntncAr[]=$prefix."'".explode(".",$mntnc)[0].".%'";
			else
				$mntncAr[]=explode(".",$mntnc)[0];
		}
		else
			$mntncAr[]=$mntnc;
	}
...
function eqconsumsionOilData($itemCode,$all=false,$unit=false){
	...
	while($row=mysqli_fetch_array($q)){
		...
		if($unit)
			$collector[]=[$row[uitemCode]=>[$itemDes[des],$itemDes[unit]] ];
		else
			$collector[]=[$row[uitemCode]=>$itemDes[des]];
	}
	return $collector;
}
...
// 	echo $sql;
	...
	$unit=measuerUnti()[$unit];
	if($unit)
		return $munit ? implode(", $extra",$r).", ".$unit : implode(",  $extra",$r);
	return false;
}
...
// 			 echo "///$dayesGone///";
			 ...
			 $ptillyesterdayWork = round(($tillyesterdayWork*100)/$qty);
			 if($rr[iowUnit]=='L.S' OR $rr[iowUnit]=='LS' OR $rr[iowUnit]=='l.s' OR $rr[iowUnit]=='l.s') $tillyesterdayWork='';
			 return [$tillyesterdayWork,$ptillyesterdayWork];
			//echo  $tillyesterdayWork.'  <font class=out>'.$rr[iowUnit].'</font>'.' ('.$ptillyesterdayWork.'%) ';
		}
		else
			return "100";
