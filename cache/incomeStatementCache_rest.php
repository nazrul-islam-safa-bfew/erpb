...
function is6802000($pcode,$fromDate,$toDate){
  global $db;
  $ch = curl_init();
...
function get680200X($fromDate,$toDate,$pcode,$fromDate1){
  $amountR=is6802000($pcode, $fromDate,$toDate);  // range
  $amountC=is6802000($pcode, $fromDate1,$toDate);  // to month  
...
// input $fromDate,$toDate,$pcode,$fromDate1
// output 
function get6803000($fromDate,$toDate,$pcode,$fromDate1){
...
ini_set("memory_limit","6000M");
ini_set("max_execution_time","960000");
// if(isCaseWagesFound($pcode)){
 	$amountR=getWagesAmount($fromDate,$toDate,$pcode);  // range
		
	if($_GET["debug"] && !$_GET["empID"]){
		echo "<meta http-equiv=\"refresh\" content=\"1;URL='http://win4win.biz/erp/bfew/cache/incomeStatementCache_rest.php?6804000=1&toDate=$toDate&pcode=$pcode&debug=1&start=".($_GET["start"]+10)."'\" /> ";
	}
...
// 	if($calculatedvalue2<0)echo "<hr style='border:2px solid red'>";
		/* if($kk++>2)
		 exit;else*/ //echo "<hr>";
		 $calAmount=$calculatedvalue>0?$calculatedvalue:$calculatedvalue2;
			if($_GET["debug"]){
				$wipSql="insert into auxiliary_wip (iowID,edate,amount) values ('$iow[iowId]','$ed','$calAmount')";
				mysqli_query($db,$wipSql);
