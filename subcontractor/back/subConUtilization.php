...
elseif($chk) {
	...
	for($i=1;$i<$n;$i++){
		...
		if($reQty<=0) break;
			$sql="INSERT INTO `subut` ( `id` ,`posl` ,`qty` , `rate`,`itemCode` , `iow` , `siow` , `details` , `edate` , `pcode` ) ".
			" VALUES ('', '$posl','${qty.$i}','$rate' ,'$itemCode', '${iow.$i}'  ,'${siow.$i}' , '', '$edate1', '$loginProject')";
			//echo '<br>'.$sql.'<br>';
			mysql_query($sql);
		}
	}//for
...
?>
<tr >
  <td ><input type="text" name="qty<? echo $i;?>" size="10"  onBlur="if(this.value><? echo $reQty;?>) {alert('You are Exceding remaining Qty!!'); this.value=0;}"> </td>
