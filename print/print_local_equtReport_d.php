...
<tr>
 <th>Equipment Report of &nbsp;<? echo myProjectName($project);?>&nbsp; at &nbsp; <? echo date('D',strtotime($edate)).'  '; echo mydate($edate); ?></th>
</tr>
</table>
<br>
<br>
...
<tr bgcolor="#EEEEEE">
 <td colspan="2"><?  echo eqpId($eqId,$itemCode); ?> <?  $temp=itemDes($itemCode); echo $temp[des].', '.$temp[spc]; ?> </td>
 <td colspan="3" align="right" valign="top"><font class='englishheadBlack'>equipment utilization report</font></td>
</tr>
<? //echo $sqlp.'<br>';?>
<tr>
...
<tr <? if(date('D',strtotime($reut[edate]))=='Fri') echo "bgcolor=#FFFFCC"; elseif($i%2==0) echo "bgcolor=#EFEFEF";?> >
  <td align="center" ><a href='./equipment/local_equtReport_byEq_detail.php?eqId=<? echo $eqId;?>&itemCode=<? echo $itemCode;?>&eqType=H&edate=<? echo $reut[edate];?>' target='_blank'>
  <? echo myDate($reut[edate]);?></a>

  <?

	$dailyworkBreakt=eq_dailyworkBreak($eqId,$itemCode,$reut[edate],'H',$project);
	
	$toDaypresent=eq_toDaypresent($eqId,$itemCode,$reut[edate],'H',$project);
	
    $toDaypresent=$toDaypresent-$dailyworkBreakt;	
	
	$workt= eq_dailywork($eqId,$itemCode,$reut[edate],'H',$project);
