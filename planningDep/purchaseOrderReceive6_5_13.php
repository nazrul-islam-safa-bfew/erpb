...
//echo "$sql<br>";
	...
	$sql = mysql_query("SELECT assetId FROM equipment WHERE itemCode LIKE '${exitemCode.$i}'");
	//echo "$sql<br>";
	$rid=mysql_num_rows($sql)+1;
	$assetId="$rid";
...
$sqlp = "SELECT distinct posl from `porder` WHERE potype in ('1','4')  AND location='$loginProject'  AND vid<>'99' AND  status=1";
...
$temp=itemDes($typel12[itemCode]);
 ?>
<? if(remainQty($posl,$typel12[itemCode],$loginProject)>0){?>
 <tr>
...
<tr >
<input type="hidden" name="n" value="<? echo $i;?>">
      <td align="center" colspan="4"> <input type="button" value="Received by <? echo $loginFullName;?>" name="epreceive1" onClick="if(checkrequired1(srf)) {srf.submit(); srf.epreceive.value=1;}"> 
        <input type='hidden' name="epreceive" value="1">
<input type="hidden" name="posl" value="<? echo $posl;?>">
