...
while($rRow=mysqli_fetch_array($rQ)){
	$data[$ii][]=$rRow['itemCode'];
	$data[$ii][]=$rRow['sdate'];
	$data[$ii][]=$rRow['revisionText'];
	$ii++;
}

if(mysqli_affected_rows($db)<1){
	echo "<center><h1>Revision not found!</h1></center>";
	exit;
}
 ...
 <td>
  ...
  <table align="left" width="98%" border="0" bordercolor="#000000" bgcolor="#FFFFEE" cellspacing="0" cellpadding="0" style="border-collapse:collapse">
    ...
    display: inline-block;    padding: 5px;    background: #ff2d2d;    color: #fff;"><?php echo poTypeCC($fsql[cc]); ?></p>
			...
			echo "<div><br><span style=\"    padding: 5px;
    background: #00f;
    color: #fff;
    border-radius: 10px;
    font-weight: 800;\">Revision #$revision:</span> ".$data[0][2]."</div>";
				?>
...
<td align="right">
<? $duration =1+(strtotime($edate)-strtotime($sdate))/86400; echo round($duration); ?> days<br>
...
if($status=='-1' OR $status=='3'  AND $loginDesignation=='Accounts Manager'){?><input type="button" onClick="location.href='./planningDep/poUnder/forwardforApproval.sql.php?posl=<? echo $posl?>&status=<? echo $status?>'" name="approved" value="Forward for Approval"><? }?>

<? if($status=='0' AND $loginDesignation=='Chairman & Managing Director'){?>
<input type="button" onClick="location.href='./planningDep/poUnder/pobackforedit.sql.php?posl=<? echo $posl?>&status=<? echo $status;?>&revision=<? echo $revision;?>'" name="approved" value="Back for Edit">
<input type="button" onClick="location.href='./planningDep/poUnder/poApprove.sql.php?posl=<? echo $posl?>&status=<? echo $status;?>&revision=<? echo $revision;?>'" name="approved" value="Approve">
<? }?>
...
if(getPOapprovedStatus($posl)=='1' AND $revision>0 AND $loginDesignation=='Chairman & Managing Director'){?>

<input type="button" onClick="location.href='./planningDep/poUnder/poApprove_revision.sql.php?posl=<? echo $posl?>&status=<? echo $status;?>&revision=<? echo $revision;?>&actionType=b'" name="approved" value="Back for Edit">

<input type="button" onClick="location.href='./planningDep/poUnder/poApprove_revision.sql.php?posl=<? echo $posl?>&status=<? echo $status;?>&revision=<? echo $revision;?>&actionType=a'" name="approved" value="Approve">
<a name="head"></a>
<? }?>