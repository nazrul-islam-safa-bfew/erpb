...
?>
 ...
 <TR>
  <TD colspan="2"><br><br>Subject: <b><? if($type=='3') echo " Work Order for "; else echo " Purchase Order for Supply of Material at";?>  <? echo myprojectName($project);?></b><br><br>
  </TD>  
 </TR>
...
?>	
    <tr>
	  ...
	  if($l==0) echo date("d-m-Y",strtotime($dd[0]));
	    ...
	    <? if($type==4) echo date("d-m-Y",strtotime($dd[0])); else echo date("d-m-Y",strtotime($dd[$m]));?></td>	  
	</tr> 
<?  } //for?>
 <tr>
 	<td > </td>
...
<tr>
<td>
 <? echo "<b>Invoice $i:</b>  Raise after supply completion ";
 ...
 <tr>
   <td>
   <? if($type==3) include('sub_conditionp.php');
	elseif($type==4)  include('eqpconditionp.php');
	else include('conditionp.php');  ?>
...
$sqlp1 = "SELECT * from  `poscheduletemp` WHERE posl='$posl' AND itemCode='$typel1[itemCode]'";
 ...
 ?>
    ...
    <tr> 
	 ...
	 echo $temp[des];
      ...
      <td align="right"><?  $duration =1+round((strtotime($edate)-strtotime($sdate))/86400); echo $duration; ?> days</td>	  
      <td align="right">Tk.<? echo number_format($typel1[rate],2)?> /nos/day   </td>
      <td align="right">Tk.<? 
	  	$sumAmount=$typel1[rate]*$duration*$typel1[qty];
		$amount+=$sumAmount;
		//echo "$sumAmount=$typel1[rate]*$duration*$typel1[qty];";
	  echo number_format($sumAmount,2)?></td>	  
    </tr>
    <?
