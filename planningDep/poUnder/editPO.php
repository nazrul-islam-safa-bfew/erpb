...
$activeDate1 = formatDate($activeDate,$format);
	  ...
	  $totalAmount=0;
	  $sq=mysqli_query($db, "SELECT poid FROM pordertemp WHERE posl='$posl'");
	  $r=mysqli_fetch_array($sq);
	  $poid=$r[poid];
	  
	  mysqli_query($db, "DELETE FROM pordertemp WHERE posl='$posl'");
	  mysqli_query($db, "DELETE FROM poscheduletemp WHERE posl='$posl'");	  
	...
	for($i=1;$i<=$n;$i++)
	{
	 ...
	 if(${itemCode.$i}<'50-00-000')$potype=1;
	  ...
	  " VALUES ('$poid','$posl','$acctPayable','$project','${itemCode.$i}','${porderQty.$i}','${rate.$i}', '$vid','-1','${qref.$i}','${deliveryDetails.$i}','$d','$activeDate1','$todat','$potype')";
	// echo "$sqlp<br>";
	...
	if($posl){
		  mysqli_query($db, "DELETE FROM pconditiontemp WHERE posl='$posl'");	 
		...
		$tch131.'_'.$tch141.'_'.$tch142.'__'.$tch151.'_'.$tch171.'_'.$tch181.'_'.
		...
		echo $jack=mysqli_affected_rows($db);
		if($jack==1) echo "good"; else echo "bad";
		
		}

	if($posl){
	mysqli_query($db, "DELETE FROM popaymentstemp WHERE posl='$posl'");	 
	$sqlpo = "INSERT INTO popaymentstemp(popID, posl, acctPayable,totalAmount,receiveAmount, paidAmount) VALUES ('','$posl','$acctPayable','$totalAmount','','')";
	//echo $sqlpo;
...
<form name="purchaseOrder" action="./index.php?keyword=edit+purchase+Order&vid=<? echo $vid;?>&project=<? echo $project;?>"  method="post">
  ...
  <table align="center" width="95%" border="1" bordercolor="#CCCCFF" cellspacing="0" cellpadding="0" style="border-collapse:collapse">
	...
	<br>
	 <?  if($activeDate=='') $activeDate=date("d/m/Y",strtotime(poreceiveActiveDate_under($posl)));?>
	Purchase Order Release date: <input type="text" name="activeDate" size="10" value="<? echo $activeDate;?>">
  <a id="anchor" href="#"
  ...
  <TD>Advance Required: <font class="out"><? echo vendorRating($vendor[advance],4);?></font></TD>
    ...
    <tr>
	 <td colspan="7">
      ...
      <th>Amount</th>
      <th>Quotation Ref.</th>
      <th bgcolor="#FFFF99">Max. PO Qty</th>
	  </tr>
...
$order=orderQty_temp($project,$typel1[itemCode],$posl);
...
//echo "totalQty=$totalQty<br>";
 ...
 $ex= $totalQty-$order;
if($ex>0){
if(${porderQty.$i}=='') ${porderQty.$i}=poQty_under($posl,$typel1[itemCode]);
?>
    <tr>
      <td><input type="text" name="porderQty<? echo $i;?>" value="<? echo ${porderQty.$i};?>" style="text-align:right" size="10"
	   onBlur="if(<? echo 'porderQty'.$i;?>.value > <? echo $ex;?> || <? echo 'porderQty'.$i;?>.value < 0 ) {
	   alert('Purchase Order quantity exceeding Estimated Quantity is not acceptable.'); <? echo 'porderQty'.$i;?>.value=0;}">
        <?  echo $temp[unit];?>
      </td>
      ...
      <td bgcolor="#FFFF99" align="right"> <?  echo number_format($ex,3); $ex=0;?></td>
    </tr>
	<?  } //if Ex?>
    <?
...
<br><br><br>
  <? if($calculate){
   if($dstart=='') $dstart=date("d/m/Y",strtotime(poreceiveStart_under($posl)));
   $sdate=poScheduleDate_under($posl);
  ?>
  <table align="center" width="1000" border="1" bordercolor="#0099FF" cellspacing="0" cellpadding="0" style="border-collapse:collapse">
    <tr> 
      <td width="300">Receiving will start at 
        <input type="text" name="dstart" size="10" value="<? echo $dstart;?>"> <a id="anchor0" href="#"
   ...
   name="anchor0" ><img src="./images/b_calendar.png" alt="calender" border="0"></a></td>
      ...
      <td  > <input type="text" name="sdate1"  size="10" value="<? if($sdate[1]) echo date("d/m/Y",strtotime($sdate[1]));?>"><a id="anchor12" href="#"
   ...
   name="anchor12" ><img src="./images/b_calendar.png" alt="calender" border="0"></a></td>
      <td  > <input type="text" name="sdate2"  size="10" value="<? if($sdate[2]) echo date("d/m/Y",strtotime($sdate[2]));?>"><a id="anchor2" href="#"
   ...
   name="anchor2" ><img src="./images/b_calendar.png" alt="calender" border="0"></a></td>
      <td  > <input type="text" name="sdate3"  size="10" value="<? if($sdate[3]) echo date("d/m/Y",strtotime($sdate[3]));?>"><a id="anchor3" href="#"
   ...
   name="anchor3" ><img src="./images/b_calendar.png" alt="calender" border="0"></a></td>
      <td > <input type="text" name="sdate4"  size="10" value="<? if($sdate[4]) echo date("d/m/Y",strtotime($sdate[4]));?>"><a id="anchor4" href="#"
   ...
   name="anchor4" ><img src="./images/b_calendar.png" alt="calender" border="0"></a></td>
      <td  > <input type="text" name="sdate5"  size="10" value="<? if($sdate[5]) echo date("d/m/Y",strtotime($sdate[5]));?>"><a id="anchor5" href="#"
   ...
   name="anchor5" ><img src="./images/b_calendar.png" alt="calender" border="0"></a></td>
      <td > <input type="text" name="sdate6"  size="10" value="<? if($sdate[6]) echo date("d/m/Y",strtotime($sdate[6]));?>"><a id="anchor<? echo $j;?>1" href="#"
   ...
   name="anchor6" ><img src="./images/b_calendar.png" alt="calender" border="0"></a></td>
      <td > <input type="text" name="sdate7"  size="10" value="<? if($sdate[7]) echo date("d/m/Y",strtotime($sdate[7]));?>"><a id="anchor7" href="#"
...
?>
    <tr <? if($j%2==1) echo "bgcolor=#EEEEEE";?> > 
      ...
      <td><input type="text" name="qty1_<? echo $j;?>" value="<? echo ${porderQty.$j}; ?>" onBlur="purchaseOrder.qty2_<? echo $j;?>.value = <? echo ${porderQty.$j}; ?> - purchaseOrder.qty1_<? echo $j;?>.value ;" style="text-align:right" size="10"> 
        <? echo $temp[unit];?>
		<? echo '<br><font class=out>'.poScheduleDateQty_under($posl,$sdate[1],$typel1[itemCode]).'</font>';?></td>
      <td><input type="text" name="qty2_<? echo $j;?>" onBlur="purchaseOrder.qty3_<? echo $j;?>.value = <? echo ${porderQty.$j}; ?> - purchaseOrder.qty1_<? echo $j;?>.value- purchaseOrder.qty2_<? echo $j;?>.value ;" style="text-align:right" size="10"> 
        <? echo $temp[unit];?>
		<? echo '<br><font class=out>'.poScheduleDateQty_under($posl,$sdate[2],$typel1[itemCode]).'</font>';?></td>		
      <td><input type="text" name="qty3_<? echo $j;?>" onBlur="purchaseOrder.qty4_<? echo $j;?>.value = <? echo ${porderQty.$j}; ?> - purchaseOrder.qty1_<? echo $j;?>.value- purchaseOrder.qty2_<? echo $j;?>.value- purchaseOrder.qty3_<? echo $j;?>.value ;" style="text-align:right" size="10"> 
        <? echo $temp[unit];?>
		<? echo '<br><font class=out>'.poScheduleDateQty_under($posl,$sdate[3],$typel1[itemCode]).'</font>';?></td>		
...
?>
   ...
   echo "pppp". $potypeTest=poType($posl);
    ...
    <tr>
      <td colspan="8">
				...
				function checkFirst(){	
					...
					for(i=1;i<=7;i++){
						...
						if(inv!="on"){ purchaseOrder.submit(); return true;}
					}
	  ...
	  <input type="button" name="save1" value="Update" onClick="save.value=1; checkFirst();">
