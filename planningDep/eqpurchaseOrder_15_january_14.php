...
$activeDate1 = formatDate($activeDate,$format);
	if($posl){
		...
		$ch28.'_'.$ch29.'_'.$ch24.'_'.$tch141.'_'.$tch142;

	   print $sql="INSERT INTO pconditiontemp(id, posl, `condition`,extra,extra1,extra2,extra3,extra4)".
...
if($r>=0){
	for($i=1;$i<=$n;$i++)
	{
	 ...
	 $d=formatDate(${dstart.'_'.$i},$format);
	  ...
	  " VALUES ('','$posl','$acctPayable','$project','${itemCode.$i}','${porderQty.$i}','${rate.$i}', '$vid','-1','${qref.$i}','${deliveryDetails.$i}','$d','$activeDate1','$todat','2')";
	//  echo "<br>** $sqlp **<br>";
	// exit;
	...
	$poid = mysql_insert_id();
		 ...
		 $fd=date('Y-m-d',$fd);
		 print $sqls = "INSERT INTO poscheduletemp(pos, posl,itemCode, sdate, qty)".
...
//old rule has been changed by suvro 13_january
...
if($vid==85)$sqlp = "SELECT itemCode,salvageValue,life,days,hours,MAX(((((price-salvageValue)/life)/days)*10)/30) as rate from `equipment` GROUP by itemCode order by itemCode ASC";
else
...
$ex= $totalQty-$order;
	//$j++;
if($ex>0){
$sql="SELECT dmaItemCode,dmaRate,SUM(dmaQty) as dmaTotal from dma WHERE  dmaProjectCode='$project' AND dmaItemCode = '".$typel1[itemCode]."'";
...
?>
    <tr>
	  ...
	  echo number_format($typel1[rate],2).'/'.$typel1[itemUnit];
	  print "</strong>";
      ...
      <td width="116"><? if(($total>0) AND ($dmarate>=$typel1[rate])){?><input type="text"  name="porderQty<? echo $i;?>" value="<? echo ${porderQty.$i};?>" style="text-align:right" size="3" maxlength="3" width="4" >
        <?  echo $temp[unit];
		}
