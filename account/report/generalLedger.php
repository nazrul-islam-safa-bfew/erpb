...
include($localPath."/employee/local_emputReport_c_for_income_satement1.php");
/*function get_normalDayAmountgTotalup_finual_plus_idle_of_total($fromDate,$toDate,$pcode)
{}
function get_normalDayAmountgTotalup_finual_for_report($fromDate,$toDate,$pcode)
{}
function get_normalDayAmountgTotalup_finual($fromDate,$toDate,$pcode)
{}
function get_total_idle_amount($fromDate,$toDate,$pcode)
{}*/
...
?>
 ...
 <tr>
    ...
    <td>From </td>
      <td>
				<input type="text" maxlength="10" name="fromDate" value="<? echo $fromDate;?>">
				<a id="anchor" href="#" onClick="cal.select(document.forms['gl'].fromDate,'anchor','dd/MM/yyyy'); return false;" name="anchor" ><img src="./images/b_calendar.png" alt="calender" border="0"></a>
      </td>
    <td>To </td>
...
if($pcode){
//echo "********pcode=$pcode************";
 if($pcode=='1'){$pcode=''; include($localPath."/account/report/hgeneralLedgerAll.php");}
  elseif($pcode=='000'){include($localPath."/account/report/headgeneralLedger.php");}
else
{ ?>
...
<!-- equipment-->
 ...
 if(myarray_search ('2407000',  $accountId)){ $crAmount=0;$drAmount=0;
 $k=0;
$openingBalance=0;

$array_date=array();
$baseOpening=baseOpening('2407000',$pcode);
$openingBalance=$baseOpening+openingBalance('2407000',$fromDate,$pcode);
  
 $db = mysqli_connect($SESS_DBHOST, $SESS_DBUSER,$SESS_DBPASS,$SESS_DBNAME);
	
	
	$sql="SELECT SUM(price) as amount, edate as todat, reference as paymentSL, itemCode,`reference` 
	FROM `equipment` 
	WHERE `edate` between '$fromDate' and '$toDate' 
	and location='$pcode'
	/*AND `reference` LIKE 'PO%' */
	GROUP BY edate,`reference` order by edate ASC ";
// echo $sql;
  $sqlQ=mysqli_query($db, $sql);
  $r=mysqli_num_rows($sqlQ);
$i=0;  
  ...
  $i++;  
	...
	$sql="1 SELECT SUM( receiveQty * rate )as amount, `todat`,`paymentSL`, `itemCode`,`reference` ".
	" FROM `storet$pcode` WHERE `todat` between '$fromDate' and '$toDate' 
	AND `paymentSL` LIKE 'ST_%'GROUP BY `paymentSL` order by todat ASC ";
...
$sql1="1 SELECT `paidAmount` as amount,`paymentSL`,`paymentDate`,`posl` from `vendorpayment` WHERE".
" `paymentDate` BETWEEN '$fromDate' AND '$toDate' AND `posl` LIKE 'PO_".$pcode."_%' Order by paymentDate ASC";
//echo $sql1;
$sqlq1=mysqli_query($db, $sql1);
while($st=mysqli_fetch_array($sqlq1)){
...
?>
 <tr>
   <? if($k==0){ ?> <td valign="top"<? echo " rowspan=$r";?> class="bg"> <? echo "2407000-$pcode<br>".accountName('2407000');?></td><? }?>
   <td valign="top"><? echo mydate($array_date[$i][0]);?></td>
   <td valign="top"><? echo $array_date[$i][1];?></td>   
   <td valign="top"><? // echo $re[reff];?></td>      
   <td> <? echo $array_date[$i][2];?></td>
...
?>
 ...
 if(myarray_search ('3301000',  $accountId)){ $crAmount=0;$drAmount=0;
 $k=0;
$openingBalance=0;

$array_date=array();
$baseOpening=baseOpening('3301000',$pcode);
$openingBalance=$baseOpening+openingBalance('3301000',$fromDate,$pcode);
  
 $db = mysqli_connect($SESS_DBHOST, $SESS_DBUSER,$SESS_DBPASS,$SESS_DBNAME);
	
	
	$sql="SELECT SUM(e.price) as amount,
	e.edate as todat,
	e.reference as paymentSL,
	e.itemCode,
	e.reference,
	e.assetId as eqID,
	i.itemDes
	FROM `equipment`  as e, itemlist as i
	WHERE e.`edate` between '$fromDate' and '$toDate' 
	and e.location='$pcode'
	/*AND `reference` LIKE 'PO%' */
	and e.itemCode=i.itemCode
	GROUP BY e.edate,e.`reference` order by e.edate ASC";
// echo $sql;
  $sqlQ=mysqli_query($db, $sql);
  $r=mysqli_num_rows($sqlQ);
$i=0;  
  ...
  $array_date[$i][0]=$st['todat'];
  $array_date[$i][1]=viewposl($st['paymentSL']);  
  $array_date[$i][2]=$st['itemCode'].$st['eqID'].": ".$st['itemDes']."; ".$st['reference'];
  $array_date[$i][3]=$st['amount'];  
  $array_date[$i][4]=1;  
  $i++;
  }
...
?>
 <tr>
   <? if($k==0){ ?> <td valign="top"<? echo " rowspan=$r";?> class="bg"> <? echo "3301000-$pcode<br>".accountName('3301000');?></td><? }?>
   <td valign="top"><? echo mydate($array_date[$i][0]);?></td>
   <td valign="top"><? echo $array_date[$i][1];?></td>   
   <td valign="top"><? // echo $re[reff];?></td>      
   <td> <? echo $array_date[$i][2]; ?></td>
...
//	$wages_TotalReceive=get_normalDayAmountgTotalup_finual_plus_idle_of_total($my_fdate,$my_fdate,$pcode);
	
//    if($wages_TotalReceive>0){	
// 	$array_date[$i][0]=$fd;
// 	$array_date[$i][1]='';  
// 	$array_date[$i][2]='Payable';
...
$sql1="select SUM(amount) as amount,`pdate`,`paymentSL` from `empsalary`
	  ...
	  AND `glCode` LIKE '2216000-$pcode' GROUP by paymentSL";
 // echo $sql.'<br>';
 echo $sql1;
$sqlq1=mysqli_query($db, $sql1);
while($st=mysqli_fetch_array($sqlq1)){
...
$i=0;	
  for($j=0;$j<$si;$j++){
	...
	$wages_TotalReceive=get_normalDayAmountgTotalup_finual($my_fdate,$my_fdate,$pcode);
	//$wages_TotalReceive=wagesAmount_date($pcode,$fd,$td);
...
?>
 ...
 <?
 if(myarray_search ('6805000',  $accountId)){
...
$openingBalance=openingBalance('6805000',$fromDate,$pcode);
...
$i=0;	
  for($j=0;$j<$si;$j++){
	...
	$wages_TotalReceive=get_total_idle_amount($my_fdate,$my_fdate,$pcode);
	//$wages_TotalReceive=wagesAmount_date($pcode,$fd,$td);
	//echo "	wages_TotalReceive=	$wages_TotalReceive<br>";
...
?>
 <tr>
   <? if($k==0){ ?> <td valign="top"<? echo " rowspan=$r";?> class="bg"> <? echo "6805000-$pcode<br>".accountName('6805000');?></td><? }?>
   <td valign="top"><? echo mydate($array_date[$i][0]);?></td>
   <td valign="top"><? echo $array_date[$i][1];?></td>   
   <td valign="top"><? // echo $re[reff];?></td>      
   <td> <? echo $array_date[$i][2];?></td>
