...
// include("./employee/wages_calc.php");
...
<!-- 	  <select name='pcode' size='1' onChange="location.href='index.php?keyword=cash+disbursment&pcode='+cdj.pcode.options[document.cdj.pcode.selectedIndex].value+'&fromDate='+cdj.fromDate.value+'toDate='+cdj.toDate.value";> -->
 ...
 <tr class="noPrint">
	 <td colspan="4" align="center">
	 		<input type="button" name="go" value="Go" onClick="gl.submit();" class="noPrint">	 		
	 </td></tr>
</table>
<br>
...
// print $Tinvoice=totalInvoiceAmount_date($pcode, $fromDate,$toDate);
 
 $rate=$iow[iowPrice]; //echo "--#";

 $TWP=(($totalMaretialCost+$totaleqCost+$totalSubconCost+$totalempCost)/$TotalExpAppPer)*100;
 
 
 // if($iow['iowUnit']=='LS'||$iow['iowUnit']=='L.S'||$iow['iowUnit']=='l.s'||$iow['iowUnit']=='ls')

		// $TWP=($workComplited-$Tinvoice)/100;
...
$i=1;
 ...
 while($iow=mysqli_fetch_array($sqlrunp))
 {
	...
	if(WP2($iow[iowId],$iow[iowProjectCode],$ed,$iow[iowQty],$iow[iowUnit],0)=='LS' || WP2($iow[iowId],$iow[iowProjectCode],$ed,$iow[iowQty],$iow[iowUnit],0)=='L.S' || WP2($iow[iowId],$iow[iowProjectCode],$ed,$iow[iowQty],$iow[iowUnit],0)=='l.s' || WP2($iow[iowId],$iow[iowProjectCode],$ed,$iow[iowQty],$iow[iowUnit],0)=='l.s')
	{
		...
		/*$workComplited2 = iowActualProgressnew($iow[iowId],$iow[iowProjectCode],$ed,$iow[iowQty],$iow[iowUnit],0); //echo "--";
			
		$workComplited=number_format($workComplited2,2,'.','');
		$invoicedQty2=invoicedQty($iow[iowId]); //echo "--";
		$invoicedQty=number_format($invoicedQty2,2,'.','');
		$rate=$iow[iowPrice]; //echo "--#";
		$unit=$iow['iowUnit'];
		if($unit=='L.S' OR $unit=='LS' OR $unit=='l.s' OR $unit=='l.s')
			$TWP+=((($workComplited-$invoicedQty)*$rate)/100); //echo "<br>"; //TWP= total work in progress
		else
		  	$TWP+=($workComplited-$invoicedQty)*$rate;*/
		...
		{
		 $workComplited3 = WP1($iow[iowId],$iow[iowProjectCode],$ed,$iow[iowQty],$iow[iowUnit],0); //echo "--";
...
//echo $sql;
  ...
  if($re[accountID]=='6801000'){
		$amount=total_mat_directissueAmount_date($pcode, $fromDate,$toDate);
 		$amount1=total_mat_directissueAmount_date($pcode, $fromDate1,$toDate1);   //current month
  }
...
$i=0;
 while($typel= mysqli_fetch_array($sqlrunp))
		...
		$a=total_eq_direct_issueAmount_date2($pcode, $fromDate,$toDate); //range
		$amount=$a;
		$_SESSION['utiamt']=$_SESSION["utilization"];//range
			
// 		$a=total_eq_direct_issueAmount_date2($pcode, $current_month,$current_day); //current month amount		
		$amount1=$a;
		$_SESSION['utiamt1']=$_SESSION["utilization"];//range
...
//   $amount1=sub_po_directReceive($pcode, $fromDate1,$toDate1); // to month 
  }  
...
if($all_list_of_data_f==1)
	$amount=$normal_dday_of_array[0];
else{
	$all_list_of_data_f=1;
	
// 	if($pcode=='195' or $pcode=='194' or $pcode=='193' or $pcode=='192' or $pcode=='189' or $pcode=='186')
// 		$normal_dday_of_array=get_ready_wages_data($toDate,$fromDate,$pcode);
// 	else
// 		$normal_dday_of_array=all_list_of_data($toDate,$fromDate,$pcode);
...
if($all_list_of_data_f==1)
	$amount=$normal_dday_of_array[1];
else{
	$all_list_of_data_f=1;
	
// 	$normal_dday_of_array=all_list_of_data($toDate,$fromDate,$pcode);
	$amount=$_SESSION[utilize_amount];
}
