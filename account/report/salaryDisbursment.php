...
<form name="cdj" action="./index.php?keyword=salary+disbursment" method="post">
<table  width="600" align="center" border="0" class="ablue" >
 <tr bgcolor="#CCCCFF">
 	<td align="right" valign="top" height="30" colspan="4" bgcolor="#0099FF">
		<font class='englishhead'>salary &amp; wages disbursment journal</font>
	</td>
</tr>
 <tr>
	 <td>Project:</td>
<!--<select name='pcode' size='1' onChange="location.href='index.php?keyword=cash+disbursment&pcode='+cdj.pcode.options[document.cdj.pcode.selectedIndex].value+'&fromDate='+cdj.fromDate.value+'toDate='+cdj.toDate.value";> -->
	...
	<tr>
		...
		<td colspan="3">
			<select name="cat">
				<option value="sal" <?php echo $cat=="sal" ? "selected" : ""; ?>>Salary</option>
				<option value="wag" <?php echo $cat=="wag" ? "selected" : ""; ?>>Wages</option>
			</select>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			<select name="desigS">
				...
				<?php
					$sql="select * from itemlist where itemcode between '70-01-001' and '85-99-999' and itemDes!='' ";
					$q=mysqli_query($db,$sql);
					while($row=mysqli_fetch_array($q)){
						if($desigS==$row[itemCode])$extt=" selected ";else $extt="";
						echo "
						<option value=\"$row[itemCode]\" $extt >$row[itemCode]-$row[itemDes]</option>
						";
					}
				?>
				<option value="" disabled style="background:#f00"> ------------------------------------------------------------------------------------------------------- </option>
				<?php
					$sql="select * from itemlist where itemcode between '86-01-001' and '91-99-999' and itemDes!='' ";
					$q=mysqli_query($db,$sql);
					while($row=mysqli_fetch_array($q)){
						if($desigS==$row[itemCode])$extt=" selected ";else $extt="";
						echo "
 ...
 --> 
  <tr>
   ...
   name="anchor" ><img src="./images/b_calendar.png" alt="calender" border="0"></a>
      &nbsp;To <input type="text" maxlength="10" name="toDate" value="<? echo $toDate;?>" > <a id="anchor2" href="#"
...
<table  width="850" align="center" border="0" cellpadding="5" cellspacing="2" style="font-size: 10px;" >
 ...
 $db = mysqli_connect($SESS_DBHOST, $SESS_DBUSER,$SESS_DBPASS,$SESS_DBNAME);	
	...
	if($cat=="sal")
		$extra=" and paymentSL like 'sp%' ";
	elseif($cat=="wag")
		$extra=" and paymentSL like 'wp%' ";
	
	if($desigS!="all")
		$extra.=" and paymentSL in (select paymentSL from empsalary where designation='$desigS') ";
		
	
  if($pcode=='000')
		$sql="select * from `purchase` WHERE paymentDate between '$fromDate' and '$toDate' 
   AND (account like '5501000-000'/* or account like '5600000%'*/) $extra order by paymentDate DESC";  
  else{
...
//  echo "<br>$reff[$i]<br>";
 ...
 <tr>
  ...
  else if(($temp[0]=='EP' OR $temp[0]=='cash') and 1==2){
...
<?	}//else
 ...
 elseif(($temp[0]=='PP' OR $temp[0]=='pp') and 1===2) { ?> 
   <? 
