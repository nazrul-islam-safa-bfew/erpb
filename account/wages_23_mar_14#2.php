...
" WHERE salaryType='Consolidated'".
" AND attendance.location='$_GET[exfor]' AND attendance.action in ('P','HP') AND employee.designation>='86-00-000' AND employee.designation<='99-00-000'".
...
<? echo $overtimeTotal1;	?>
   ...
   <td  align="right" ><? $otAmount=otRate($re[salary],$fromD)*($overtimeTotal/3600);
...
onClick="if(this.checked==true){currentPayable<? echo $b;?>.alt='cal';p.value=p.value+'_<? echo $b;?>'} else {currentPayable<? echo $b;?>.alt=''}" onchange="disableSave(this.form);"></td>   	      
 ...
 <tr>
  ...
  myOnclick('<? echo $exfor;?>','<? echo $month;?>',this.form);"></td>
   <td colspan="11" align="right"> Total Amount: <input type="text" readonly="" name="total" value='0' style=" border:0;background: #FFFFCC;text-align:right"></td>
 </tr>
  <tr>
...
<input type="hidden" name="chk">
 <input type="button" value="Print" onClick="mygoto('<? echo $exfor;?>','<? echo $month;?>',payments.chk.value,'<? echo $year;?>');">
