...
<? ${remainAmount.$i}=round($re[totalAmount]-$re[paidAmount],2);?>
 <tr>
   ...
   <td align="right">
    ...
    $approved_amount="select sum(amount) as amount from vendorPaymentApproval where posl='$re[posl]' AND location='$location'";
...
-->
  ...
  style="text-align:right"  <? if(${poPayableAmount.$i}<=0 )echo ' disabled ';?>
   onBlur="if(<? echo 'amountPaid'.$i;?>.value > <? echo round(${poPayableAmount.$i},2);?>)
    {alert('Amount exceeds PO Total'); <? echo 'amountPaid'.$i;?>.value=0;}" value="" onchange="disableSave(this.form);" onClick="do_it(<?php echo $row_approved['amount'].','.$i ?>);"  >  
