...
<SCRIPT language=JavaScript>document.write(getCalendarStyles());</SCRIPT>
 ...
 <form name="sitestore" action="index.php?keyword=store" method="post">
<table align="center" width="500" class="vendorTable">
<tr >
 <td colspan="2" align="right" valign="top" class="vendorAlertHd">site store Search form</td>
</tr>

<tr><td >Project</td>
<td><select name="pcode">
 <option value="">Center Store</option>
<?
include("config.inc.php");
$db = mysqli_connect($SESS_DBHOST, $SESS_DBUSER,$SESS_DBPASS,$SESS_DBNAME);
	

$sqlppg = "SELECT * from `project` where pcode<='008' ";
if($loginProject!='000')
	$sqlppg.=" and pcode in ('$loginProject')";
$sqlppg.=" order by pcode asc";
// echo $sqlppg;
 ...
 while($typelpg= mysqli_fetch_array($sqlrunppg))
{
 ...
 if($pcode==$typelpg[pcode]) echo " selected ";
...
<tr><td>Item Code</td>
    <td >
	<input name="itemCode1" onKeyUp="return autoTab(this, 2, event);" size="2" maxlength="2" value="<?php echo $itemCode1; ?>" > - 
    <input name="itemCode2" onKeyUp="return autoTab(this, 2, event);" size="2" maxlength="2" value="<?php echo $itemCode2; ?>"> - 
    <input name="itemCode3" onKeyUp="return autoTab(this, 3, event);" size="3" maxlength="3"  value="<?php echo $itemCode3; ?>">
	</td>
</tr>
 <tr>
...
<tr><td colspan="2" align="center"  class="vendorAlertHd">
<input type="submit" name="search" value="Search" class="vendor" onMouseOver="this.className='vendorhov'" onMouseOut="this.className='vendor'">
</td></tr>
</table>
</form>
<? //$pcode=$pcode;
...
?>
<table align="center" width="95%" border="3" class="vendorTable" >
<tr >
 <td colspan="8" align="right" valign="top"  class="vendorAlertHd">site store status</td>
</tr>
<tr >
...
$sql="SELECT DISTINCT itemCode FROM store$pcode WHERE 1  ";	
...
$qty_pending=mat_stock_pending($pcode,$sqlresult[itemCode],$toDate,$toDate);
$temp=itemDes($sqlresult[itemCode]);
 echo $temp[des].', '.$temp[spc];?>
   <?php if($qty_pending[store] || $qty_pending[issue]){ ?>
  <br>   
   <b>Pending Qty: PO <font color="#f00"><?php echo $qty_pending[store]; ?></font>, Issue <font color="#f00"><?php echo $qty_pending[issue]; ?></font> <?php echo $temp[unit]; ?> </b>
    <?php } ?>
   
  </td>
 <td  align="center"><? echo $temp[unit];?></td>
 <td  align="right"><? 
	if($qty<0)echo "<font color='#f00'>";
	echo number_format($qty,3);
	if($qty<0)echo "</font>";?></td>
 <td align="right" >
<? 
	if($amount<0)echo "<font color='#f00'>";
	
	if($amount<0)$newAmount=$amount * (-1);
	else $newAmount=$amount;
	echo number_format($newAmount,3);
//	echo number_format($amount,2);
	if($amount<0)echo "</font>";
  echo "<br>";
  echo "<a href='./issue_finder.php?itemcode=$sqlresult[itemCode]&pcode=$pcode' class='fix_this'>Fix This</a>";
?></td>
<? 
...
<? }?>
<tr>
<TD  align='right'  class="vendorAlertHd_lite" colspan="7">
 <? echo "Total Stock Value Taka: <b>"; echo number_format($total,2);?></b></TD>
</tr >
...
$(document).ready(function(){
  $("a.fix_this").click(function(){
    $.get($(this).attr("href"),function(data){
      if(data){
        alert("Done, Please search again.");
      }else{
