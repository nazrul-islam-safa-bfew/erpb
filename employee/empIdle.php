...
if($h1 AND $m1 AND $h2 AND $m2){
	$stime=$h1.':'.$m1.':00';
	$etime=$h2.':'.$m2.':00';
	if(!isUtilized($assetId,$edate1,$stime,$etime)){
		$sql = "INSERT INTO `equt` ( `id` , `assetId` , `iow` , `siow` , `stime` , `etime` , `details` , `edate`,`pcode` ) ".
						   "VALUES ('', '$assetId', '', '', '$stime', '$etime', '$remark', '$edate1','$loginProject')";
		//echo $sql.'<br>';
		mysqli_query($db, $sql);
	}		
	else $err=1;
...
<tr bgcolor="#CC9999">
 <td colspan="2" align="right" valign="top"><font class='englishhead'>equipment break down entry form</font></td>
</tr>
<tr>
 <th>From</th>
 <th>To</th> 
