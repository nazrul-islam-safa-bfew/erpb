...
$sqlp="SELECT DISTINCT attendance.empId,designation,name,salary,allowance FROM attendance,employee
 WHERE employee.designation>='90-00-000' AND employee.designation<='97-00-000'
...
<? echo $overtimeTotal1;	?>
   ...
   ?></td>
    ...
    <td align="right"> 
      <? $amount=$otAmount+$atAmount; echo number_format($amount,2);
	  $paid=currentPayble($re[empId],"$year-$month-01");
	  $payable= $amount-$paid;
	 // echo "**$payable= $amount-$paid;**";
	  ?>
	<input type="hidden" name="currentPayable<? echo $b;?>" value="<? echo round($amount,2);?>" alt="">
   </td>   
   <td valign="top" align="center">
   <? if($payable<=0){ echo " Paid "; }else{?>
   <input type="checkbox" name="ch<? echo $b;?>"    value="<? echo $re[empId];?>" <? if(${ch.$b}) echo 'checked';?> 
onClick="if(this.checked==true){currentPayable<? echo $b;?>.alt='cal';p.value=p.value+'_<? echo $b;?>'} else {currentPayable<? echo $b;?>.alt=''}"></td>   	      
 <? }//else?>
 </tr> 
 <? 
