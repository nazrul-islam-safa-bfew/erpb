...
if($loginProject!='000') $project=$loginProject;
 $sqlq="SELECT * FROM employee WHERE designation < '89-00-000' AND location='$project' AND status=0 AND empDate<='$dd' and empId not in (select empId from appaction where action='1' and details='Termination from Job') ORDER by designation";
// echo $sqlq;
 ...
 $page.='<a href="" onmousemove="this.href=\'./index.php?keyword=attendance&project='.$project.'&d=\'+document.attendance.d.value+\'&page='.$pl.'\'">'.$pl."</a> | ";}
	
	$last_one=(100+(100*$get_page_loc));
	$new_sql=$sqlq." limit ".$get_page_loc*100 ." , ".$last_one;
