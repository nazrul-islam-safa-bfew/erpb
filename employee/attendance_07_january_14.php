...
if($loginProject!='000') $project=$loginProject;
 $sqlq="SELECT * FROM employee WHERE designation < '87-00-000' AND location='$project' AND status=0 AND empDate<='$dd' and empId not in (select empId from appaction where action='1' and details='Termination from Job')  ORDER by designation";
// echo $sqlq;
