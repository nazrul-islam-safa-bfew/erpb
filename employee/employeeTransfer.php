...
$sqlp = "SELECT * from `employee` WHERE location='$transferFrom' AND status=0 order by designation ASC";
//echo $sqlp;
$sqlrunp= mysqli_query($db, $sqlp);
$i=1;
...
?>
	<tr <? if($i%2==0) echo  "bgcolor=#EEEEEE";?>>
	  <td align="left"><input type="checkbox" name="ech<? echo $i;?>" id="ech<? echo $i;?>" value="<? echo $emp[empId];?>" <? if($c) echo " disabled"; ?>>
	  <input type="hidden" name="designation<? echo $i;?>" id="designation<? echo $i;?>" value="<? echo $emp[designation]?>">
	  <?                
	...
	$lastAtt=getLastAttendance($emp[empId]);
	if($lastAtt!="0000-00-00" && $lastAtt)
		echo "<br>Last Ateendance: ".$lastAtt;
	
	    ?>
		<input type="hidden" name="plocation<? echo $i;?>" id="plocation<? echo $i;?>" value="<? echo $temp1[transferTo];?>" />
		<input type="hidden" name="tid<? echo $i;?>"  id="tid<? echo $i;?>" value="<? echo $temp1[tid];?>" />
...
function data_reset(n)
{
	for(i=1;i<n;i++)
	{
		if(document.getElementById("ech"+i).checked==0)
		{
			document.getElementById("ech"+i).remove();
			document.getElementById("designation"+i).remove();
			document.getElementById("plocation"+i).remove();
			document.getElementById("tid"+i).remove();
		}
	}
	empTransfer.submit();
}
...
<input type="hidden" name="n" value="<? echo $i;?>" id="n">
<tr><td colspan="2" align="center"><input type="button" name="teansfer" id="dll" value="Transfer" onclick="data_reset(document.getElementById('n').value)"  />
