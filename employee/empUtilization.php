...
include_once('./wages_calc.php'); //wages function
require_once("../keys.php");

if(!$_SESSION["loginProject"]){
...
if($rr!=1)	{
	for($i=1;$i<$n;$i++){
		...
		$stime=${h1.'_'.$i}.':'.${m1.'_'.$i};
		$etime=${h2.'_'.$i}.':'.${m2.'_'.$i};
	    if(emp_isUtilized($empId,$empD,$edate1,$stime,$etime,$empType)){
		...
		$reQty= empRemainHr($empD,${iow.$i},${siow.$i});
		if($reQty<=0) break;
		$diff=(${h2.'_'.$i}*3600+${m2.'_'.$i}*60)-(${h1.'_'.$i}*3600+${m1.'_'.$i}*60);
		...
		`iow` , `siow` , `stime` , `etime` , `details` , `edate`,pcode, supervisor)
		 ...
		 '$stime', '$etime', '${details.$i}', '$edate1','$loginProject','$supervisor')";
// 		echo $sql.'<br>';
		mysqli_query($db, $sql);
...
if($empId && $edate1 && $loginProject){	$_GET["empID"]=$_GET["start"]=$_GET["debug"]=$_GET["pcode"]=$_GET["edate"]=$_GET["toDate"]=null;	
	$wagesCalUrl="../cache/incomeStatementCache_rest.php?6804000=1&toDate=$edate1&pcode=$loginProject&debug=1&start=0&empID=$empId&edate=$edate1"; //automation for wages calculation
		$_GET["edate"]=$_GET["toDate"]=$edate1;
		$_GET["pcode"]=$loginProject;
		$_GET["debug"]=1;
		$_GET["start"]=0;
		$_GET["empID"]=$empId;
...
// 		echo "$_GET[empID] && $_GET[edate] && $_GET[start] && $_GET[debug] && $_GET[pcode] && $_GET[toDate]";
		...
		if($_GET["empID"] && $_GET["edate"] && isset($_GET["start"]) && $_GET["debug"] && $_GET["pcode"] && $_GET["toDate"])
			{
				getWagesAmount($edate1,$edate1,$loginProject);
				echo "<p>Wages has been calculated.$response</p>";
			}
}
	
?>

<form name="equt" action="./empUtilization.php?<? echo "siow=$siow&empId=$empId&empD=$empD&edate=$edate&empType=$empType";?>" method="post">
...
" WHERE iow.iowprojectCode='$loginProject' AND siow.siowId=dma.dmasiow AND siow.siowId='$siow' AND dmaItemCode='$empD'".
...
<? if($ck) echo "<td colspan=2 align=center><font class=out> Completion Date Expired.</font></td>";else {?>
  <td align="center" >
	  <input name="h1_<? echo $i?>" value="<? if($chk && $rr) echo ${h1.'_'.$i};?>"  
	  size="2" maxlength="2" onKeyUp="return autoTab(this, 2, event);"> :
	  <input name="m1_<? echo $i?>" value="<? if($chk && $rr) echo ${m1.'_'.$i};?>" 
	  size="2" maxlength="2" onKeyUp="return autoTab(this, 2, event);">
  </td>
  <td align="center">
		<div style="display:inline-block;">
			
	  <input name="h2_<? echo $i?>" value="<? if($chk && $rr) echo ${h2.'_'.$i};?>" 
	  size="2" maxlength="2" onKeyUp="return autoTab(this, 2, event);"> :
	  <input name="m2_<? echo $i?>" value="<? if($chk && $rr) echo ${m2.'_'.$i};?>" 
	  size="2" maxlength="2" > 	
  ...
  <input type="hidden" name="reQty<? echo $i;?>" value="<? echo $reQty;?>">  
	<? }?>  
<? if($i==1){?>	
  <td align="right" rowspan="3"><? echo sec2hms($reQty/3600,$padHours=false);?> Hrs.  </td>  
 ...
 <tr bgcolor="#99CCFF">
  ...
  <td align="center">
	  ...
	  <input name="m2_<? echo $i?>" value="<? if($chk && $rr) echo ${m2.'_'.$i};?>"  size="2" maxlength="2">
		...
		<div style="display:inline-block;">
			Work Break: <input type="text" name="details" width="100" size="21" value="Lunch">
		</div>
  </td>
  <td align="right"></td>  
  <td></td>  
...
<input type="hidden" name="n" value="<? echo 5;?>">
...
<tr <? if($i%2==0) echo "bgcolor=#EFEFEF";?> >
  ...
  <td align="center"><a href="<? echo $PHP_SELF."?siow=$siow&empId=$empId&empD=$empD&edate=$edate&delete=1&id=$reut[id]&empType=$empType";?>">[ Delete ]</a></td>  
 </tr>
 <? $i++;}?>
</table>
<p align="center">
