...
?>
<form name="store" action="index.php?keyword=released+employee+Details&page=1" method="post">
<table align="center" width="400" class="human" >
<tr  >
 <td colspan="2" align="right" valign="top" class="humanHd">released human resource Search form</td>
</tr>
...
if($loginDesignation=='Construction Manager')$sqlp.=" and itemcode in (select designation from release_employee where ccr='$getItemDesCode' and location='$managerPcode') ";
...
$sql="SELECT * FROM release_employee as employee WHERE status=0";
...
// else 
$sql.=" AND  designation BETWEEN '71-00-000' AND '97-99-999' ";  
if($loginProject!='000') $location=$loginProject;
if($location AND $location!='Select one'){$sql.=" AND employee.location='$location' ";}
	
if($managerFound && $loginDesignation!='Human Resource Manager')$sql.=" AND employee.ccr='$getItemDesCode' ";

if($itemCode){
	$sql.=" AND employee.designation LIKE '$itemCode'";
}
	
$sql.=" AND employee.empId in (select empId from appaction where action='1' and actionStatus='0' and date1!='')";
...
<tr bgcolor="#E9D1D1">
 ...
 <?   echo '<b>'.hrDesignation($sqlresult[designation]).'</b>';?>
</td>
<td>
<a target="_blank" href="./print/print_createJob.php?designation=<? echo $test;?>">Job Description</a>
</td> 
<td>
<a href="./index.php?keyword=appraisal&id=<? echo $sqlresult[empId];?>"> New Appraisal</a> || 
...
while($app=mysqli_fetch_array($sqlaq)){
 $temp=appraisal($sqlresult[empId],$app[appId])?>
<tr bgcolor="#E9D1D1">
  <td colspan="3" >
...
<!--   <a target="_blank" href="./index.php?keyword=appraisal&appId=<? echo $app[appId];?>"><? echo $temp[rate];?></a> -->
  <i>Appraised by <? echo $temp[supervisor];?> on <? echo date("F d, Y",strtotime($app[todat]));?></i>
  </td>
</tr>
<? }//app?>
...
$testp= $test;
 ...
 <?php
        ...
        $range= 5;
		$rootLink="./index.php?keyword=released+employee+details";
