...
$ending="$xh:$xm:00";
	for($i=1;$i<$n;$i++){
		if(${h1.'_'.$i}){
				$stime=${h1.'_'.$i}.':'.${m1.'_'.$i};
				$etime=${h2.'_'.$i}.':'.${m2.'_'.$i};
   			if(eq_isUtilized($eqId,$itemCode,$edate1,$stime,$etime,$eqType)){$err[$i]=1;  $rr=1;continue; }
...
if($rr!=1)	{
	for($i=1;$i<$n;$i++){
		...
		if($reQty<=0)  break;
		$sql = "INSERT INTO `equt` ( `id` , `eqId` ,`itemCode`, `iow` , `siow` , `stime` , `etime` , `details` , `edate`,pcode,posl ) ".
						   "VALUES ('', '$eqId','$itemCode', '${iow.$i}', '${siow.$i}', '$stime', '$etime', '', '$edate1','$loginProject','$posl')";
		//echo $sql.'<br>';
		}
        elseif(${iow.$i}=='-1'){$sql = "INSERT INTO `equt` ( `id` , `eqId` ,`itemCode`, `iow` , `siow` , `stime` , `etime` , `details` , `edate`,pcode,posl ) ".
						   "VALUES ('', '$eqId','$itemCode', '-1', '-1', '$stime', '$etime', '${details.$i}', '$edate1','$loginProject','$posl')";
		//echo $sql.'<br>';
		}
		
		else{
		$sql = "INSERT INTO `equt` ( `id` , `eqId` ,`itemCode`, `iow` , `siow` , `stime` , `etime` , `details` , `edate`,pcode,posl ) ".
						   "VALUES ('', '$eqId','$itemCode', '', '', '$stime', '$etime', '${details.$i}', '$edate1','$loginProject','$posl')";
		//echo $sql.'<br>';
		} 
		//echo $sql.'<br>';
		mysqli_query($db, $sql);
...
<form name="equt" action="./eqUtilization_ws.php?<? echo "eqId=$eqId&itemCode=$itemCode&edate=$edate&eqType=$eqType&posl=$posl";?>" method="post">
...
$sqlppg = "SELECT iow.*,siow.*,dma.dmasiow from iow,`siow`,dma WHERE ".
...
<? if($ck) echo "<td colspan=2 align=center><font class=out> Completion Date Expired.</font></td>";else {?>
  ...
  <input type="hidden" name="reQty<? echo $i;?>" value="<? echo $reQty;?>">    
	<? $i++;}?>    
  <td align="right"><? echo sec2hms($reQty/3600,$padHours=false);?> Hrs.</td>  
...
<tr <? if($i%2==0) echo "bgcolor=#EFEFEF";?> >
  ...
  <td align="center"><a href="<? echo $PHP_SELF."?eqId=$eqId&itemCode=$itemCode&edate=$edate&delete=1&id=$reut[id]&eqType=$eqType";?>">[ Delete ]</a></td>  
 </tr>
 <? $i++;}?>
</table>
<p align="center">
