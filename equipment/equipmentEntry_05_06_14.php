...
<tr bgcolor="#FFEEEE">
   ...
   <td ><input type="text" size="5" name="days" value="<? echo $eqresult[days];?>" <? if($r) echo 'readonly';?>> Months   </td>
</tr>
<tr>
   <td>Daily Working Hours</td>
   <td ><input type="text" size="5" name="hours" value="<? echo $eqresult[hours];?>" <? if($r) echo 'readonly';?>> Hours   </td>
</tr>
<tr bgcolor="#FFEEEE">
...
<tr >
	<td >Current price</td>
		...
		<?php
			...
			$total_time_y=date("Y",strtotime($eqresult[mnfPro]))+$eqresult[life];
			$total_time_md=date("m-d",strtotime($eqresult[mnfPro]));
			...
			//echo ($total_time-strtotime(date("Y-m-d")) ) / 86400;
			//-$total_time;
	...
	//total time 
	//echo 
	//echo strtotime($total_time);
		// ((-strtotime(date('Y-m-d')))/3600)/24;
