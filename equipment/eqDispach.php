...
$dat=formatDate($edate,"Y-m-d");
	for($i=0;$i<$n;$i++){	
		if(${chk.$i}){$sqlp = "UPDATE `eqproject` set edate='$dat',dispatch='$dispachTxt',status=2 WHERE id=${chk.$i}";
		//echo $sqlp.'<br>';
...
<form name="eq" action="./index.php?keyword=eq+dispach&e=<? echo $e;?>" method="post">
...
/*if($loginProject=='002') $sql = "SELECT * FROM equipment WHERE location IN ('002','004') ORDER BY assetId asc";
else $sql = "SELECT * FROM equipment WHERE location='$loginProject' ORDER BY assetId asc";
*/
$sql = "SELECT * FROM eqproject WHERE posl LIKE 'EQ_".$loginProject."_%' AND status='1' ORDER BY posl asc";
echo $sql;
//echo $loginProject;
...
<? $i++; } //while?>
<tr><td colspan="4">Challan No. & Delivary Details: <input type="text" name="dispachTxt" width="100" size="100"></td></tr>
</table>
</td>
</tr>
<input type="hidden" name="n" value="<? echo $i;?>">
</table>
<p align="center"><input  type="submit" name="Dispach" value="Dispach"></p>
</form>
<div id=testdiv1
