...
if($rr!=1)	{
	for($i=1;$i<$n;$i++){
		...
		$etime=${h2.'_'.$i}.':'.${m2.'_'.$i};
		if(${iow.$i}){
		$sql = "INSERT INTO `emput` ( `id` , `empId` ,`empType`,`designation`, `iow` , `siow` , `stime` , `etime` , `details` , `edate`,pcode ) ".
						   "VALUES ('', '$empId','$empType','$empD', '${iow.$i}', '${siow.$i}', '$stime', '$etime', '', '$edate1','$loginProject')";
		//echo $sql.'<br>';
		}
		else{
		$sql = "INSERT INTO `emput` ( `id` , `empId` ,`empType`,`designation`, `iow` , `siow` , `stime` , `etime` , `details` , `edate`,pcode ) ".
						   "VALUES ('', '$empId','$empType','$empD', '${iow.$i}', '${siow.$i}', '$stime', '$etime', '$details', '$edate1','$loginProject')";
		//echo $sql.'<br>';
		} 
		mysqli_query($db, $sql);
...
//	$sql="UPDATE attendance set over1='$eh',over2='$em',over1='$xh',over1='$xm' WHERE empId=$empId AND edate='$edate1'";
	//$sql = "UPDATE `attendance` SET over1='$eh',over2='$em',over1='$xh',over1='$xm' WHERE `empId` =$empId AND `edate` = '$edate1' ";
	
	$sql="UPDATE `attendance` SET `over1` = '$eh',`over2` = '$em',`over3` = '$xh',`over4` = '$xm' WHERE `empId` =$empId AND `edate` = '$edate1'" ;
    //echo $sql.'<br>';
...
$sqlppg = "SELECT iow.*,siow.*,dma.dmasiow from iow,`siow`,dma WHERE iow.iowprojectCode='$loginProject' AND siow.siowId=dma.dmasiow AND dmaItemCode='$empD' AND  siow.iowId=iow.iowId AND iowStatus LIKE 'Approved%'";
//echo $sqlppg;
...
//echo "C=$chk=R:$rr";
?>
<tr <? if($err[$i]==1) echo "bgcolor=#FFCCCC";?>>
  <td align="center">
  ...
  <td>
<? echo '<font color=006600>'.iowCode($typelpg[iowId])."$typelpg[siowCode]; </font>$typelpg[siowName]";?>
