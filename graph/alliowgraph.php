...
if($r==2){$graph->subtitle->Set("(with SIOW Progress)");}
...
$sql1 = mysql_query("SELECT * FROM iow WHERE iowProjectCode='$gproject' AND iowStatus != 'Not Ready' LIMIT 0, 10"); 
$j=0;
...
$sql = mysql_query("SELECT * FROM siow WHERE iowId='$rowiow[iowId]'"); 
$i=1;
$j++;
...
// Format the bar for the first activity
// ($row,$title,$startdate,$enddate)
$activity = new GanttBar($j,"--$siowName[$i]","$siowSdate[$i]","$siowCdate[$i]","[$complete[$i]%]");
...
// Specify progress to 60%
$a = $complete[$i]/100;
$activity->progress->Set($a);
$activity->progress->SetPattern(GANTT_SOLID,"darkgreen");
$graph->Add($activity);
$i++;
$j++;
}//siow
...
<table width="100%">
 <tr><td>
	<p align="left"><img align="middle" src="./alliowgraph.php" alt="Graph"></p> 
 </td></tr>
