...
if ($cfg['SkipLockedTables'] == TRUE) {
    ...
    $db_info_result = PMA_mysqli_query($db, $local_query) or PMA_mysqlDie('', $local_query, '', $err_url_0);
            ...
            if ($db_info_result != FALSE && mysql_num_rows($db_info_result) > 0) {
                while ($tmp = PMA_mysql_fetch_row($db_info_result)) {
                    if (!isset($sot_cache[$tmp[0]])) {
                        ...
                        $sts_result  = PMA_mysqli_query($db, $local_query) or PMA_mysqlDie('', $local_query, '', $err_url_0);
                        $sts_tmp     = PMA_mysqli_fetch_array($sts_result);
                        $tables[]    = $sts_tmp;
                    } else { // table in use
...
if (!isset($sot_ready)) {
    ...
    $db_info_result = PMA_mysqli_query($db, $local_query) or PMA_mysqlDie('', $local_query, '', $err_url_0);
    if ($db_info_result != FALSE && mysql_num_rows($db_info_result) > 0) {
        while ($sts_tmp = PMA_mysqli_fetch_array($db_info_result)) {
            $tables[] = $sts_tmp;
        }
        mysql_free_result($db_info_result);
    }
