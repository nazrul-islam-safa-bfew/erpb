...
function PMA_fieldTypes($db, $table,$use_backquotes) {
    ...
    $table_def = PMA_mysqli_query($db, 'SHOW FIELDS FROM ' . PMA_backquote($db) . '.' . PMA_backquote($table));
    while($row = @PMA_mysqli_fetch_array($table_def)) {
...
function PMA_getTableDef($db, $table, $crlf, $error_url, $show_dates = false)
{
    ...
    $result = PMA_mysqli_query($db, 'SHOW TABLE STATUS FROM ' . PMA_backquote($db) . ' LIKE \'' . PMA_sqlAddslashes($table) . '\'');
    if ($result != FALSE) {
        if (mysql_num_rows($result) > 0) {
            $tmpres        = PMA_mysqli_fetch_array($result);
    ...
    if ($use_backquotes) {
        PMA_mysqli_query($db, 'SET SQL_QUOTE_SHOW_CREATE = 1');
    } else {
        PMA_mysqli_query($db, 'SET SQL_QUOTE_SHOW_CREATE = 0');
    }
    $result = PMA_mysqli_query($db, 'SHOW CREATE TABLE ' . PMA_backquote($db) . '.' . PMA_backquote($table));
...
function PMA_exportData($db, $table, $crlf, $error_url, $sql_query)
{
    ...
    $result      = PMA_mysqli_query($db, $sql_query) or PMA_mysqlDie('', $sql_query, '', $error_url);
    if ($result != FALSE) {
