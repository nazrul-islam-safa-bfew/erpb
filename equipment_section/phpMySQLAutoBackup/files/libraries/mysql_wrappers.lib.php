...
function PMA_mysql_fetch_row($result) {
     ...
     * speed really matters, duplicate here code from PMA_mysqli_fetch_array
...
function PMA_mysqli_query($db, $query, $link_identifier = FALSE, $result_mode = FALSE) {
    if ($link_identifier != FALSE) {
        if ($result_mode != FALSE) {
            return mysqli_query($db, PMA_convert_charset($query), $link_identifier, $result_mode);
        } else {
            return mysqli_query($db, PMA_convert_charset($query), $link_identifier);
        }
    } else {
        return mysqli_query($db, PMA_convert_charset($query));
    }
}
...
function PMA_mysql_list_tables($database_name, $link_identifier = FALSE) {
    if ($link_identifier != FALSE) {
        return PMA_mysqli_query($db, 'SHOW TABLES FROM ' . PMA_backquote(PMA_convert_charset($database_name)), $link_identifier);
    } else {
        return PMA_mysqli_query($db, 'SHOW TABLES FROM ' . PMA_backquote(PMA_convert_charset($database_name)));
    }
}
...
function PMA_mysql_list_fields_alternate($database_name, $table_name, $link_identifier = FALSE) {
    if ($link_identifier != FALSE) {
        $result = PMA_mysqli_query($db, 'SHOW FIELDS FROM '
    ...
    $fields = array();
    while ($row = PMA_mysqli_fetch_array($result)) {
        $fields[] = $row;
    }
