...
function PMA_safe_db_list($only_db_check, $dbh, $dblist_cnt, $rs, $userlink, $cfg, $dblist) {
    ...
    if ($only_db_check == FALSE) {
        ...
        if (PMA_MYSQL_INT_VERSION >= 40002) {
            ...
            $rs               = PMA_mysqli_query($db, $local_query, $dbh); // Debug: or PMA_mysqlDie('', $local_query, FALSE);
        ...
        if ($is_safe_show_dbs && strtoupper($is_safe_show_dbs) != 'OFF') {
            ...
            while ($uva_row = PMA_mysqli_fetch_array($uva_alldbs)) {
                  $dblist[] = $uva_row[0];
            } // end while
        ...
        if (!$dblist_cnt) {
            ...
            $rs           = PMA_mysqli_query($db, $auth_query, $dbh); // Debug: or PMA_mysqlDie('', $auth_query, FALSE);
        } // end
    }
    ...
    if (!$dblist_cnt
        ...
        if ($row['Select_priv'] != 'Y') {
            ...
            if ($rs && @mysql_numrows($rs)) {
                ...
                while ($row = PMA_mysqli_fetch_array($rs)) {
...
if ($is_minimum_common == FALSE) {
        ...
        if ($dblist_cnt) {
            ...
            for ($i = 0; $i < $dblist_cnt; $i++) {
                ...
                if ($is_show_dbs && ereg('(^|[^\])(_|%)', $dblist[$i])) {
                    ...
                    $rs          = PMA_mysqli_query($db, $local_query, $dbh);
    ...
    function PMA_countRecords($db, $table, $ret = FALSE)
    {
        ...
        $result       = PMA_mysqli_query($db, $local_query) or PMA_mysqlDie('', $local_query, '', $err_url);
        $showtable    = PMA_mysqli_fetch_array($result);
        ...
        if (!isset($num)) {
            $result = PMA_mysqli_query($db, 'SELECT COUNT(*) AS num FROM ' . PMA_backquote($db) . '.' . PMA_backquote($table)) or PMA_mysqlDie('', $local_query, '', $err_url);
...
//-->
        ...
        // Corrects the tooltip text via JS if required
            ...
            $result = @PMA_mysqli_query($db, 'SHOW TABLE STATUS FROM ' . PMA_backquote($GLOBALS['db']) . ' LIKE \'' . PMA_sqlAddslashes($GLOBALS['table'], TRUE) . '\'');
            if ($result) {
                $tbl_status = PMA_mysqli_fetch_array($result, MYSQL_ASSOC);
...
//-->
        ...
        if (isset($GLOBALS['table']) && isset($GLOBALS['sql_query'])
            ...
            if (isset($tbl_status) && (int) $tbl_status['Index_length'] > 1024) {
                @PMA_mysqli_query($db, 'REPAIR TABLE ' . PMA_backquote($GLOBALS['table']));
            }
        }
        unset($tbl_status);
