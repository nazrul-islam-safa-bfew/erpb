...
if($loginDesignation!="Project Manager")	
	while($p=mysqli_fetch_array($sqlQ1)){
	...
	?>
	 <tr <? if($in[invoiceNo]==$invoiceNo) echo "bgcolor=#FFFFCC";?> >
	   <td><? if($in[invoiceType]==2){?><a href="./index.php?keyword=view+invoice&invoiceNo=<? echo wp_entities($in[invoiceNo], 1);?>"><? echo $in[invoiceNo];?></a><? }
	  ...
	  <? if($in[invoiceStatus]==1){?> <a href="#" onClick='if(confirm("Are you sure ?")) window.location="./index.php?keyword=view+invoice&invoiceNo=<? echo wp_entities($in[invoiceNo], 1);?>&d=1"'><img src="./images/b_drop.png" border="0" align="absbottom"></a> <? } else { echo "";} ?>
	   </td>	      
	 </tr>
...
else
{
	...
	$sql="SELECT * from invoice WHERE invoiceLocation ='$loginProject' AND (invoiceStatus='1' or invoiceStatus='2')
	ORDER by invoiceLocation,invoiceDate ASC";
	 //echo $sql;
	$sqlQ=mysqli_query($db,$sql);	
	$r=mysqli_num_rows($sqlQ);
	...
	?>	
	<tr class="vendorAlertHdt" >
		<td colspan="9" align="left"><? echo '['.$loginProject.'] '.$p[pname];?></td>
	</tr>
	<? 
	while($in=mysqli_fetch_array($sqlQ)){
	?>
...
$i=1;
while($in=mysqli_fetch_array($sqlrunp)){
 $sql1="SELECT * FROM iow WHERE iowId=$in[iowId]";
$sql1q=mysqli_query($db,$sql1);
$iow=mysqli_fetch_array($sql1q);
?>
<tr>
