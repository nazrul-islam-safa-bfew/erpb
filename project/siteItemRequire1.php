...
//echo $sql;
	...
	{
		$startrow=0;
		$rowperpage=floor($totalrow/2);
	}
	else if ($page=="2")
	{
		$startrow=floor($totalrow/2);
		$rowperpage=$totalrow-floor($totalrow/2);
	}
$sql="SELECT dmaItemCode,dmaProjectCode,SUM(dmaQty) as dmaTotal,iowStatus,iow.iowId from dma,iow WHERE".
"  iowStatus IN ('Approved by Mngr P&C','Approved by MD') AND iow.iowId=dma.dmaiow AND dmaProjectCode='$pcode'";

if($type=='mat')
$sql.=" AND dmaItemCode BETWEEN '01-00-000' AND '49-99-999' Group by dmaItemCode limit 201, $tot";
$sqlquery=mysql_query($sql);
...
<? //}//pcode?>
<b>Page: <a href="./index.php?keyword=site+item+require&type=mat">Previous</a> | <a href="./index.php?keyword=site+item+require1?page=2">Next</a></b>